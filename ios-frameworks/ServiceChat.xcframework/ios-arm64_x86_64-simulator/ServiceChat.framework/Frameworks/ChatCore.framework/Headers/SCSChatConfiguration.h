...
/**
 The eventlist which describes a set of special URLs which will trigger an event the application can listen to
 to allow custom app interactions through the chat feed.

 Events will be fired when a user presses the associated button on the feed when we detect a registered URL.
 */
@property (nonatomic, readwrite, copy) SCSAppEventList *eventList;
